...
from estim8.workers import Worker, FederatedWorker, run_worker_service, run_worker_pool, init_logging
...
def test_run_worker_service(test_estimator):
    host = "127.0.0.1"
    port = 9500
    p = multiprocessing.Process(target=run_worker_service, args=(host, port, test_estimator))
    try:
        p.start()
        time.sleep(5)
        client = pytensor_federated.common.LogpServiceClient(host, port)
        res = client.evaluate([0])
        np.testing.assert_almost_equal(test_estimator.objective_for_replicate(None, None, None), res)
    finally:
        p.terminate()
        p.join()
    pass
